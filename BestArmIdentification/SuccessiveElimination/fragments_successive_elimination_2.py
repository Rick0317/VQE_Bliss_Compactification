...
def process_single_rotation(args):
    """
    Process a single rotation and append data directly to final JSON files
    """
    rot_idx, (i_rot, j_rot, a_rot, b_rot), H_qubit, cisd_wavefunction, n_qubits, mol, lock = args
    ...
    if lock is not None:
        with lock:
            # Append eigen_info to the final file
            with open(f"eigen_info_{mol}.json", 'a') as f:
                for eigen_data in eigen_info_rot:
                    json.dump(eigen_data, f)
                    f.write(',\n')

            # Append allocation_info to the final file
            allocation_data = {
                'generator_idx': rot_idx,
                'allocation': allocation.tolist()  # Convert to list for JSON serialization
            }
            with open(f"allocation_info_{mol}.json", 'a') as f:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Initialize the final JSON files
        ...
        with mp.Pool(processes=n_processes) as pool:
            results = pool.map(process_single_rotation, [
                (rot_idx, (i_rot, j_rot, a_rot, b_rot), H_qubit, cisd_wavefunction, n_qubits, mol, lock)
                for rot_idx, (i_rot, j_rot, a_rot, b_rot) in enumerate(valid_combinations)
            ])

        # Clean up the JSON files by removing trailing commas and closing arrays
        with open(f"eigen_info_{mol}.json", 'rb+') as f:
            f.seek(-2, 2)  # Go to second-to-last character
            f.truncate()   # Remove trailing comma and newline
            f.write(b'\n]')  # Close array

        with open(f"allocation_info_{mol}.json", 'rb+') as f:
            f.seek(-2, 2)  # Go to second-to-last character
            f.truncate()   # Remove trailing comma and newline
            f.write(b'\n]')  # Close array
        ...
        results = []
        for rot_idx, (i_rot, j_rot, a_rot, b_rot) in enumerate(valid_combinations):
            result = process_single_rotation((rot_idx, (i_rot, j_rot, a_rot, b_rot), H_qubit, cisd_wavefunction, n_qubits, mol, None))
