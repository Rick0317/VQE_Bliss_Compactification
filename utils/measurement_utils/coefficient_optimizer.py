...
def optimize_coeffs(pw_grp_idxes_fix, pw_grp_idxes_no_fix_len, meas_alloc, shara_pauli_only_decomp, shara_pauli_only_no_fixed_decomp, pw_indices_dict, psi, n_qubits, original_decomp, alpha):
    """
        ...
        for spw_idx, pws in enumerate(fragment.terms):
            ...
            for tpw_idx, pwt in enumerate(shara_pauli_only_decomp[frag_idx].terms):
                ...
                if pwt in shara_pauli_only_no_fixed_decomp[frag_idx].terms:
                    matrix[row_idx, pw_idx_dict[frag_idx]] -= cov_st_paulis.real/meas_a
            ...
            for kpw_idx, kpw in enumerate(shara_pauli_only_decomp[fixed_group_index].terms):
                ...
                if kpw in shara_pauli_only_no_fixed_decomp[fixed_group_index].terms:
                    matrix[row_idx, pw_idx_dict[fixed_group_index]] += cov_ks_paulis.real / \
                                                              meas_alloc[
                                                                  fixed_group_index]
...
def get_split_measurement_variance_unconstrained(coeff, no_group, sharable_paulis_list, sharable_paulis_fixed_list, sharable_no_fixed_decomp, pw_grp_idxes_no_fix, new_grp_idx_start, H_q, n_qubits, wfs, Ne):
    ...
    for pw_idx, pw in enumerate(sharable_paulis_list):
        ...
        for grp_idx in pw_grp_idxes_no_fix[pw]:
            coefficient -= coeff[new_grp_idx_start[grp_idx] + qubit_op_to_list(sharable_no_fixed_decomp[grp_idx]).index(pw)]
        ...
        for grp_idx in pw_grp_idxes_no_fix[pw]:
            ...
            for igp, group_word in enumerate(copy_measured_groups[grp_idx].terms):
                if (group_word == pw):
                    ...
                    measured_groups[grp_idx] += (coeff[new_grp_idx_start[
                                                          grp_idx] + qubit_op_to_list(
                       sharable_no_fixed_decomp[grp_idx]).index(pw)]) * QubitOperator(
                       pw)
                    added = True

            if not(added): measured_groups[grp_idx] += coeff[new_grp_idx_start[grp_idx] + qubit_op_to_list(sharable_no_fixed_decomp[grp_idx]).index(pw)] * QubitOperator(pw)
        ...
        if not (sharable_paulis_fixed_list[pw_idx] == None):
           ...
           for igp, group_word in enumerate(copy_measured_groups[fixed_idx].terms):
               if (group_word == pw):
                  ...
                  added = True
           if not(added): measured_groups[fixed_idx] += fixed_grp_coefficients[pw_idx] * QubitOperator(pw)
...
def get_meas_alloc_update(groups, variances, n_qubits, alpha=0.01):
   ...
   sqrt_vars = np.abs(np.sqrt(np.array(alpha * avg_vars + (1. - alpha) * np.array(variances) )))
...
if __name__ == '__main__':
    ...
    original_exp = expectation(gso(H_q, N), psi)

    print(f"Correct Expectation value: {original_exp}")
    ...
    sharable_paulis_fixed_list = [indices[-1] for indices in sharable_pauli_indices_list]
    ...
    print(expectation_v)
    assert np.isclose(expectation_v.real, original_exp.real, atol=1E-6), "Expectation value shouldn't change"

    print(f"Updated variance {variance}")

    print(meas_alloc)

    var, coeff, grp_var, fixed_grp_coefficients, measured_groups = CS_heuristic_mol(
        meas_alloc,
        decomp,
        sharable_paulis_list,
        sharable_paulis_fixed_list,
