...
from BLISS.generator_based_BLISS.generator_based_bliss_optimization import bliss_three_body_indices_filtered_non_overlap
from OperatorPools.generalized_fermionic_pool import *
from StatePreparation.parametrized_state import get_single_unitary_parametered_state
import numpy as np
from Screenings.flip_based_screening import get_non_zero_anti_hermitian_two_body
import os
import csv
from openfermion import is_hermitian, hermitian_conjugated
...
def commutator_variance(H: FermionOperator, decomp, N, psi):
    ...
    for i, frag in enumerate(decomp):
        vars[i] = np.real(variance(gso(frag, N), psi))
        print(f"Variance: {vars[i]}")
...
if __name__ == '__main__':
    ...
    com_name_list = [
        (5, 6, 7, 5), (0, 5, 7, 0), (4, 2, 0, 5), (2, 3, 1, 4), (6, 0, 6, 0), (0, 6, 7, 4)
    ]
    ...
    for i in range(len(anti_com_list)):
        ...
        print(f"Is Hermitian H? {is_hermitian(H)}")
        H_in_q = ferm_to_qubit(H)
        print(f"Is Hermitian H_q? {is_hermitian(H_in_q)}")

        j, b = 4, 2
        hf_state = get_single_unitary_parametered_state(j, b, N, Ne)

        norm_v = np.linalg.norm(hf_state)
        ...
        bliss_output = bliss_three_body_indices_filtered_non_overlap(copied_H2, N, Ne, j, b)
        bliss_output = (bliss_output + hermitian_conjugated(bliss_output)) / 2
        print(f"Is Hermitian H_BLISS? {is_hermitian(bliss_output)}")
        ...
        print(f"Is Hermitian H_BLISS_q? {is_hermitian(H_bliss_q)}")
        ...
        file_name = f"strong_killer_bliss_one_U_result_march14_{mol_name}.csv"
        ...
        with open(file_name, mode='a' if file_exists else 'w', newline='',
            ...
            # Write the data
            writer.writerow(
                ["Some A", original_var, blissed_vars, one_norm,
                 blissed_one_norm])
