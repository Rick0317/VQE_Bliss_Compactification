...
def _fermion_operator_to_majorana_operator(
    ...
    Args:
        fermion_operator: The input FermionOperator.
...
def _fermion_term_to_majorana_operator(term: tuple) -> CustomMajoranaOperator:
    """
    ...
    for index, action in term:
        converted_op = CustomMajoranaOperator((2 * index,), 0.5)

        if action:
            converted_op += CustomMajoranaOperator((2 * index + 1,), -0.5j)
